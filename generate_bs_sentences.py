    ...
    sentences = list(
        map(
            lambda sentence: create_sentence_variation(
                sentence, similar_word_number=similar_word_replacement_number
            ),
            tqdm(bs_sentences),
        )
    )
    ...
    for token in tokens:
        if (
            ...
            new_word = random.choice(
                most_similar_words(token.text, n=similar_word_number)
            )
...
def cleanup_sentences(sentences):
    ...
    new_sentences = list(
        map(lambda sentence: rewrite_sentence(sentence=sentence)["answer"], sentences)
    )
