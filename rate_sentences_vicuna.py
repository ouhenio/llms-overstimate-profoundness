...
MODELS = { "7b": "lmsys/vicuna-7b-v1.5", "13b": "lmsys/vicuna-13b-v1.5" }
...
def inference(input_text: str, temperature: float, tokenizer, model) -> str:
    inputs = tokenizer(input_text, return_tensors="pt").to(device)
    outputs = model.generate(**inputs, temperature=temperature, max_new_tokens=1)
    result = tokenizer.batch_decode(outputs, skip_special_tokens=True)[0].strip()
    inferred_text = result.removeprefix(input_text.strip())
...
Below is a sentence and its score in the format:

Sentence | Score
{sentence} | """

    # print(prompt)
    output = inference(prompt, temperature, tokenizer, model)
    # print('out', output)
    score = int(output)
    return score
...
def run_with_retries(
    ...
    tokenizer = LLaMATokenizer.from_pretrained(MODELS[model_name], use_fast=True)
    ...
    while True:
        try:
            rate_sentences(tokenizer, model, output_file)
            break
        except Exception as e:
